...
Status on **2026-09-08**: implementation extracted into both independent packages and verified through local links and installed archives. The maintainer additionally confirmed that both packages were verified through symlinks in the source application. Release preparation is authorized through commits, `v0.1.0` tags and remote pushes for both repositories. **Registry publication and Satis build/upload remain explicitly deferred.** Consumer READMEs describe the public Packagist/npm installation contract, not evidence of completed registry registration.
...
- These are independent Git repositories and versioned packages, not a shared release train. Both initial releases use version `0.1.0` and tag `v0.1.0`. npm targets public npmjs.org access; the Composer manifest version must match its tag. Configured GitHub origins do not establish registry ownership.
- The user authorized extraction, incremental commits, public dependency downloads, MIT licensing, distribution preparation, and now release commits, tags and remote pushes after source-app symlink verification. Running Satis or `npm publish` remains explicitly excluded. Copyright: 2026 Tomáš Vojík. The reference application's license remains unchanged.
...
- Source-app symlink verification is maintainer-reported acceptance evidence. It is accepted without re-running the application verification or modifying its local links, source, dependencies or lockfiles.
...
7. **Git release preparation authorized; registry publication deferred.** The workspace `satis.json` includes the `lsr/text-catalog` VCS repository. Both packages target `0.1.0` / `v0.1.0`; commit, tag and push are authorized after maintainer-confirmed source-app symlink verification. npm's public tarball includes TypeScript sources for its source maps. Packagist registration is separate from Satis configuration. Registry credentials/ownership and published-package verification remain future gates. No Satis build/upload or npm publication is authorized.
...
Git release preparation uses version `0.1.0` and tag `v0.1.0` in each independent repository. Before a later, explicitly authorized registry publication, confirm registry identity/credentials, use the tagged revisions, rebuild and inspect the archives, and review the platform observations above. Register the PHP repository on Packagist separately from any authorized Satis build/upload. Do not run Satis or `npm publish` as part of Git release preparation. After publication, install the published versions in a consumer and verify that transition; keep the source application's existing local setup unchanged until that work is authorized. Neither app's current framework constraints should be tightened or advanced merely to match the reference application.
