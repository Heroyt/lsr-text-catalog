...
Existing LSR sibling packages use MIT. The reference application's Composer and npm manifests declare proprietary licensing. Skeleton metadata deliberately withholds a redistribution grant (`proprietary` / `UNLICENSED`); this is not the intended final license decision. Confirm permission to extract and choose the package license before copying implementation or fixtures. Do not copy the game's license, catalog content or full dependency set.
...
1. **Approve extraction prerequisites.** Confirm code/fixture redistribution rights, final package names and registry/scope ownership. Use the existing GitHub origins above and pin the reference revision. Choose license and dependency support matrices; use the README dependency inventories as candidates, not tested promises. Exit: recorded approvals and supported target versions, with no app upgrades assumed.
